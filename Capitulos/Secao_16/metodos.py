...
Em Python dividimos os métodos em dois grupos:
 ...
 # Métodos de Instâncias - (Precisamos de uma instância do objeto para poder fazer acesso a ele)

 # O método init __init__ é um método especial chamado de construtor e sua função é construir o objeto
 a partir da classe
 ...
 ATENÇÃO: Por mais que possamos criar nossas proprias funções utilizando dunder (underline no inicio e no fim)
 não é aconselhavel. Python possui varios metodos com esta forma de nomenclatura e pode ser que mudemos o comportamento
 dessas funções magicas da linguagem. Então, evite ao maximo. De preferencia nunca o faça.
...
print(Usuario.nome_completo(user1))  # Estamos usando a classe para executar, não estamos usando o self(não é instancia)
...
sobrenome = input('Inform o sobrenome: ')
