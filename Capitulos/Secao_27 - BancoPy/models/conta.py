...
class Conta:
    ...
    def __init__(self: object, cliente: Cliente) -> None:
        ...
        self.saldo_total: float = self._calcula_saldo_total
        Conta.codigo += 1
    ...
    def sacar(self: object, valor: float) -> None:
        if 0 < valor <= self.saldo_total:               # Mesma comparação da linha 80
            if self.saldo >= valor:
                self.saldo = self.saldo - valor
                self.saldo_total = self._calcula_saldo_total
            else:
                restante: float = self.saldo - valor
                self.limite = self.limite + restante        # Como restante será negativo, precisamos somar e não subtrair do self.limite
                self.saldo = 0
                self.saldo_total = self._calcula_saldo_total
